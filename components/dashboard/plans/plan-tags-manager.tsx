...
interface PlanTagsManagerProps {
  onClose: () => void;
  onUpdate: () => void;
}

export function PlanTagsManager({ onClose, onUpdate }: PlanTagsManagerProps) {
  ...
  async function fetchTags() {
    ...
    // Fetch all plans to count usage
    ...
    const tagList: TagUsage[] = planTags?.map((tag) => ({
      name: tag.name,
  ...
  async function handleDeleteTag(tagName: string) {
    ...
    const message = tag.count > 0
      ? `This will remove the "${tagName}" tag from ${tag.count} plan${tag.count !== 1 ? 's' : ''} and delete it from the library. Continue?`
    ...
    // If tag is used in plans, remove it from them first
    if (tag.count > 0) {
      ...
      // Update each plan to remove this tag
      const updates = plans
        ?.filter((plan) => plan.tags?.includes(tagName))
        .map(async (plan) => {
          const updatedTags = plan.tags.filter((t: string) => t !== tagName);
      ...
      if (hasErrors) {
        alert('Some plans failed to update');
        return;
      }
    }

    // Delete from plan_tags table
  ...
  async function saveEditTag(oldName: string) {
    ...
    // Update the tag in plan_tags table
    ...
    // Update each plan to rename this tag
    const updates = plans
      ?.filter((plan) => plan.tags?.includes(oldName))
      .map(async (plan) => {
        const updatedTags = plan.tags.map((t: string) => t === oldName ? newName : t);
        const { error } = await supabase
          .from('training_plans')
          .update({ tags: updatedTags })
          .eq('id', plan.id);
      ...
      <div className="bg-[#0A0A0A] border border-white/10 rounded-2xl max-w-2xl w-full max-h-[80vh] flex flex-col">
        ...
        <div className="p-6 border-b border-white/10">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h2 className="text-2xl font-bold text-white">Plan Tags Manager</h2>
              <p className="text-sm text-gray-400 mt-1">
                Manage tags for categorizing training plans
              </p>
            </div>
            <button
              onClick={onClose}
            ...
            <div className="space-y-4">
              ...
              {showCreateTag ? (
                <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4">
                  <h4 className="text-white font-semibold mb-3">Create New Plan Tag</h4>
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="e.g., beginner, advanced, off-season"
                      />
                    </div>
              ...
              {tags.length === 0 && !showCreateTag ? (
                <div className="text-center py-12">
                  <p className="text-gray-400 text-lg">No plan tags yet</p>
                  <p className="text-gray-500 text-sm mt-2">
                    Click the button above to create your first plan tag
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                      ...
                      {editingTag === tag.name ? (
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="text-xs text-gray-400">
                            Used by {tag.count} plan{tag.count !== 1 ? 's' : ''}
                          </div>
                          <div className="flex gap-2 pt-2">
                            <button
