...
export async function updateSession(request: NextRequest) {
  ...
  const protectedPaths = ['/dashboard', '/athlete'];
  ...
  if (user && request.nextUrl.pathname.startsWith('/sign-')) {
    ...
    // For now, redirect coaches/admins to dashboard, athletes to athlete page
    // You can enhance this logic based on the user's profile
    url.pathname = '/dashboard'; // Default to dashboard
    return NextResponse.redirect(url);
  }
